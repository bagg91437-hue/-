...
// Client-side fallback API Key verification when server returns 404 (e.g. Vercel)
async function verifyKeyDirectly(apiKey: string) {
  ...
  for (const model of modelsToTry) {
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: "Confirm key" }] }]
        })
      });
      ...
      if (
        ...
        throw new Error(`유효하지 않은 API Key입니다: ${errMsg}`);
      }
      
      lastError = new Error(errMsg);
    } catch (err: any) {
      if (err.message?.includes("API_KEY_INVALID") || err.message?.includes("유효하지 않은 API Key")) {
        throw err;
      }
      lastError = err;
    }
  ...
  throw lastError || new Error("API Key 검증에 실패했습니다.");
}

// Client-side status check helper that gracefully falls back if server is 404 (e.g. Vercel static)
async function checkApiStatus(): Promise<{ hasApiKey: boolean; message: string }> {
  try {
    ...
    return {
      hasApiKey: !!localKey,
      message: localKey 
        ? "로컬 브라우저에 등록된 API Key가 작동 대기 중입니다 (서버 독립 모드)." 
        : "서버가 오프라인이거나 Vercel 정적 모드로 가동 중입니다. 하단에서 직접 Key를 등록하십시오."
    };
  }
}

// Client-side fallback Gemini Counselor prompt executor
async function callGeminiDirectlyForCounsel(apiKey: string, text: string, mode: string) {
  let resolvedMode = mode || "AUTO";
  const tagMatch = text.match(/\[MODE:\s*(PRE_SURVEY|CLASSIFY|LOG|PSYCH_INTERPRET|SERVICE_LINK)\]/i);
  if (tagMatch) {
    resolvedMode = tagMatch[1].toUpperCase();
  } else if (resolvedMode === "AUTO") {
    if (text.includes("점수") || text.includes("검사") || text.includes("프로파일")) {
      resolvedMode = "PSYCH_INTERPRET";
    } else if (text.includes("설문") || text.includes("사전 정보") || text.includes("희망직무")) {
      resolvedMode = "PRE_SURVEY";
  ...
  if (resolvedMode === "PRE_SURVEY") {
    ...
    ${text}
    `;
  } else if (resolvedMode === "PSYCH_INTERPRET") {
    responseMimeType = "application/json";
    textPrompt = `
    ...
    ${text}
    `;
  } else if (resolvedMode === "SERVICE_LINK") {
    responseMimeType = "application/json";
    textPrompt = `
    ...
    - 직업훈련 연계 검토 요인: 희망 분야의 핵심 자격 결핍, 기술 향상 전환, 경력 단절로 지식 보완이 시급함.
  ...
  const systemInstruction = `당신은 대한민국 고용노동부 산하 공공 고용센터에서 근무하는 직업상담사를 보조하는 전문적인 '초기상담 보조 AI 어시스턴트'입니다.
...
8. 제공되는 모드별 규격 포맷을 어기지 말고, 키와 구조가 일치하는 JSON 또는 요청된 규격 텍스트로 결과를 충실히 출력하십시오.`;
  ...
  for (const model of modelsToTry) {
    try {
      console.log(`[Client Fallback Gemini] Attempting model: ${model}`);
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`;
      const payload = {
      ...
      console.warn(`[Client Fallback Gemini] Model ${model} failed:`, err);
      lastError = err;
    }
  }

  throw lastError || new Error("직접 AI 서비스를 호출하는 데 실패했습니다. API Key 상태 및 네트워크 연결을 점검하십시오.");
}
...
export default function App() {
  ...
  const handleVerifyApiKey = async (rawKey: string) => {
    ...
    try {
      ...
      try {
        ...
        if (response.status === 404 || text.includes("NOT_FOUND") || text.includes("could not be found")) {
        ...
        console.warn("서버 검증 실패/404 감지로 클라이언트 직접 연결을 활용합니다:", serverErr.message);
        isClientFallback = true;
      }

      if (isClientFallback) {
        await verifyKeyDirectly(trimmed);
        data = {
          valid: true,
          message: "클라이언트 브라우저에서 직접 Google Gemini API Key를 성공적으로 검증했습니다! (Vercel 정적 모드 작동 중)"
        };
      }
      ...
      setVerifyError(err.message || "API Key 검증 중 오류가 발생했습니다. 공식 Key 형식을 확인해 주세요.");
      setVerifiedKey(null);
  ...
  const triggerAnalysis = async () => {
    ...
    try {
      ...
      try {
        ...
        if (response.status === 404 || text.includes("NOT_FOUND") || text.includes("could not be found")) {
          ...
          try { errData = JSON.parse(text); } catch (e) {
            errData = { error: `서버 에러가 발생했습니다. (상태: ${response.status})` };
          }
        ...
        if (verifiedKey) {
          console.warn("서버 분석 실패/404 감지로 클라이언트 직접 분석을 수행합니다:", serverErr.message);
          isClientFallback = true;
        } else {
          throw serverErr;
        }
      ...
      if (isClientFallback) {
        if (!verifiedKey) {
          throw new Error("서버 환경이 비활성화되어 있어 분석을 진행할 수 없습니다. 화면 우하단의 키 입력을 이용해 주십시오.");
        }
        resData = await callGeminiDirectlyForCounsel(verifiedKey, inputText, activeMode);
      }
            ...
            <button
              onClick={() => {
                checkApiStatus()
                  ...
                  .catch(() => {
                    alert("상태 조회 중 오류가 발생했습니다. 아래 입력창을 통한 직접 입력을 권장합니다.");
                  });
              }}
